...
function outputFeed(data, callback) {
  var lineSets = [buildHeaderLines(data), data, buildTerminatingLines(data)]
...
function buildHeaderLines(data) {
  return [['HDR', argv.mid, argv.mname, moment().format('YYYY-MM-DD/HH:mm:ss')]];
}

function buildTerminatingLines(data) {
  return [['TRL', data.length]];
}
